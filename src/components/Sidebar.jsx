...
function Sidebar({ personalInfo, aboutMe, skills, information }) {
  ...
  return (
    // REMOVED overflow-y-auto, kept sticky, removed h-screen from previous steps
    <aside className="w-80 md:w-96 flex-shrink-0 space-y-6 sticky top-0 p-4 md:p-0">
      ...
      <div className={firstCardClasses}>
        ...
        <div className="relative h-48 bg-cover bg-center bg-[url('/assets/sidebar-background.jpg')]">
          {/* Gradient Overlay */}
          <div className="absolute inset-0 bg-gradient-to-b from-sky-500/30 via-sky-500/10 to-white/60"></div>
          {/* Profile Picture */}
          <img
        ...
        <div className="pt-16 px-6 pb-6">
           ...
           {/* Download CV Button */}
           <a
             href={personalInfo.cvUrl} download target="_blank" rel="noopener noreferrer"
      ...
      {/* Card 2: About Me & Contact */}
        <div className={cardClasses}>
          ...
          <p className="text-sm text-gray-600 leading-relaxed mb-4"> {aboutMe} </p>
           <div className="pt-4 border-t border-gray-100">
             <div className="flex items-center justify-center space-x-6"> {/* Centered social icons */}
               {/* Email */}
               <a href={`mailto:${personalInfo.email}`} className="text-gray-500 hover:text-blue-600 transition duration-150 ease-in-out">
               ...
               {/* LinkedIn */}
               {personalInfo.linkedinUrl && ( // Conditionally render
                 <a href={personalInfo.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-blue-600 transition duration-150 ease-in-out">
               ...
               {/* GitHub */}
               {personalInfo.githubUrl && ( // Conditionally render
                 <a href={personalInfo.githubUrl} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-blue-600 transition duration-150 ease-in-out">
                   <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"> <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/> </svg>
                 </a>
               )}
             </div>
           </div>
